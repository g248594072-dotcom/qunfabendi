...
服务器不再提供完整操作台，只做「接收资料 + 无头发送」。

1. **本机** `npm run ui` → http://127.0.0.1:3789/  
2. 登录助手完成账号 / 代理 / 登录  
3. 主控制台完成：同步主页 → 探测 → 黑名单 → 同步客户 → 保存文案与勾选主页  
4. 填写「推送到服务器发送」：服务器地址（如 `https://qunfa.guanligongju.vip`）、账号密码  
5. 点 **「推送到服务器并发送」**  
6. 可选：打开服务器状态页看日志（精简页，不是操作台）
