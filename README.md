...
1. **本机** `npm run ui` → 打开 http://127.0.0.1:3789/login.html  
   添加账号、设代理、登录 Facebook（本机会弹 Chrome）  
2. 本机主控制台完成：同步主页 → 探测 → 黑名单 → 同步客户 → 保存设置  
3. 点 **「一键导出全部资料包」**（登录助手或主控制台均可）  
4. **服务器**控制台点 **「一键导入资料包」**，勾选无头模式并保存  
5. 服务器只点 **⑥ 真正发送**
